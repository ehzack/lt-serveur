...
from settings import PATH_PM_SCHNEIDERELECTRICIM221
import math
import datetime

from logmanagement import logmanagement
log = logmanagement.getlog('SchneiderElectriciM221', 'powermeter').getLogger()

class SchneiderElectric_iM221():
	...
	def load_modbus_map(self):
		filename = PATH_PM_SCHNEIDERELECTRICIM221
		self.modbusmap = {}
		i = 0
		separator = ";"
	...
	def _modbusRead(self, address,count,mb_funcall,type):
		#print("address,mb_funcall,type",address,mb_funcall,type)
		val = self.mb.readRegistersAndDecode(registri=int(address),counter=count,mb_funcall=mb_funcall,mb_type=type)
		# log.debug('address:',address,"Value",val)
		#print("val::",val)
#0 1 uint16 3 False
		return val
	...
	def _modbusWrite(self, command, base_address, value, mb_funcall, force=False):
		...
		skip_encode = False
		val = self.mb.writeRegisters(base_address, value, mb_funcall, force, skip_encode=skip_encode)
	...
	def cmd_set_date_time(self, key='Set DateTime', data=datetime.datetime.today()):
		...
		print("payload_processed: ",payload_processed)
	...
	def cmd_set_value(self,address,value=1,mb_funcall=5):
		log.debug('Start "cmd %s' % (address))
	
		self._modbusWrite(command=address,base_address=address, value=value, mb_funcall=mb_funcall, force=True)
		# log.debug('End "cmd_set_date_time" routine: %s' % ret)
