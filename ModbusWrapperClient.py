...
class ModbusWrapperClient():
	def __init__(self, modbusUnitAddress, maxRegsRead, modbusTimeout, endian="auto"):
		self.client = None
	...
	def check_address(self, address):
		# print("self.validaddresses:",self.validaddresses)
	...
	def readRegistersAndDecode(self, registri, counter,  mb_funcall=3, mb_type='int64',force=False):
		tmp = None
	...
	def decode(self, raw, size, mb_type, mb_funcall=3):
		...
		if mb_funcall == 1:
			...
			decoder = BinaryPayloadDecoder.fromCoils(raw, byteorder=self.endian, wordorder=self.endian)
		elif mb_funcall == 2:
			...
			decoder = BinaryPayloadDecoder.fromRegisters(raw, byteorder=self.endian, wordorder=self.endian)
		elif mb_funcall == 4:
			# Read Input Registers (FC=04)
